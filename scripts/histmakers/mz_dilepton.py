...
def build_graph(df, dataset):
    ...
    df = muon_selections.select_good_muons(df, args.pt[1], np.iinfo(np.int64).max, dataset.group, nMuons=2, use_trackerMuons=args.trackerMuons, use_isolation=passIsoBoth, isoBranch=isoBranch, isoThreshold=isoThreshold, requirePixelHits=args.requirePixelHits)
